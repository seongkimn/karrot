...
const REVIEWS: { likes: number; author: string; text: string }[] = [
  { likes: 9, author: "오리", text: "사진을 못 찍었네요ㅠㅠ 집 가는 길에 본 고양이..." },
  { likes: 8, author: "쉬리", text: "분명 뜨개 모임 나갔는데~ 전 수다만 ㅋㅋㅋ 지난..." },
  { likes: 9, author: "제라늄", text: "오랜만에 모뜨 넘 즐거웠어요^^" },
  { likes: 6, author: "뜨실이", text: "다음에 또 함께 떠요! 즐거운 시간이었습니다 🧶" },
];
...
const SCHEDULES: { month: string; day: string; title: string; locked?: boolean; time: string; people: string }[] = [
  ...
  { month: "5월", day: "2", title: "모임에만 공개된 일정이에요.", locked: true, time: "오후 12:00", people: "4/7명" },
];
...
export default function ScheduleFeed({ onBack }: { onBack: () => void }) {
  return (
    <Box
      style={{
        position: "fixed",
